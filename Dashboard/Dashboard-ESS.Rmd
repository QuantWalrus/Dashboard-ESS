...
datos_mariposa <- ess_data %>%
  ...
  mutate(
    ...
    # Asegúrate de tener tu vector nombres_paises definido previamente
  ...
  # --- IMPORTANTE: AGRUPAR TAMBIÉN POR CNTRY ---
  # Al incluir cntry aquí, se conservará en el resultado final
  group_by(cntry, Nombre_Pais, Region) %>% 
  summarise(
    Media_Trad = mean(Horas_Tradicional, na.rm = TRUE),
    Media_Net = mean(Horas_Internet, na.rm = TRUE),
    .groups = 'drop' # Esto desagrupa al final para evitar advertencias
  ) %>%
...
# --- PASO 1: CREAR EL CEREBRO COMPARTIDO (SharedData) ---

grupo_id <- "Link_Mariposa_Scatter"

# SharedData para la Mariposa (Datos Agregados)
sd_mariposa <- SharedData$new(datos_mariposa, key = ~cntry, group = grupo_id)

# SharedData para el Scatter (Datos Detallados)
sd_scatter <- SharedData$new(datos_scatter_full, key = ~cntry, group = grupo_id)

# Este filtro controla a todo el grupo 'Link_Mariposa_Scatter'
filtro_region <- filter_select(
  id = "region",
  label = "Selecciona una Región:",
  sharedData = sd_mariposa,
  group = ~Region,
  multiple = TRUE
)
...
max_horas <- ceiling(max(max(datos_mariposa$Media_Net), max(datos_mariposa$Media_Trad)))
tick_vals <- seq(-max_horas, max_horas, by = 1)
tick_text <- abs(tick_vals)

grafico_mariposa <- plot_ly(sd_mariposa, y = ~Nombre_Pais) %>%
  add_trace(x = ~Media_Net, name = 'Internet', type = 'bar', orientation = 'h',
            marker = list(color = '#E74C3C')) %>%
  add_trace(x = ~Media_Trad_Neg, name = 'Tradicional', type = 'bar', orientation = 'h',
            marker = list(color = '#2C3E50'),
            customdata = ~Media_Trad,
            hovertemplate = '%{y}<br>Tradicional: %{customdata:.2f} h<extra></extra>') %>%
  layout(
    barmode = 'overlay',
    title = "Consumo de Medios por País",
    xaxis = list(title = "Horas (Izq: Tradicional | Der: Internet)",
                 tickmode = 'array', tickvals = tick_vals, ticktext = tick_text,
                 range = c(-max_horas*1.1, max_horas*1.1)),
    yaxis = list(title = ""),
...
# BLOQUE 1: Filtro y Mariposa
bscols(
  widths = c(3, 9),
  list(
    htmltools::h3("Filtros"),
    filtro_region,
    htmltools::p("Selecciona 'Norte', 'Sur', etc. para resaltar esos países.")
  ),
  grafico_mariposa  # <--- AQUÍ SOLO LA MARIPOSA
)
...
grafico_scatter <- plot_ly() %>%
  # -- GRUPO 1: CONFIANZA --
  add_markers(data = sd_scatter, x = ~Horas_Jitter, y = ~Trust_Jitter,
              marker = list(size = 5, color = "#2c3e50", opacity = 0.3), # Opacity baja para resaltar selección
              name = "Encuestados", visible = TRUE) %>%
  add_lines(data = grid_horas, x = ~Horas_Internet, y = ~Pred_Trust,
            line = list(color = "#e74c3c", width = 3), name = "Tendencia", visible = TRUE) %>%
  
  # -- GRUPO 2: JUSTICIA --
  add_markers(data = sd_scatter, x = ~Horas_Jitter, y = ~Fair_Jitter,
              marker = list(size = 5, color = "#2980b9", opacity = 0.3),
              name = "Encuestados", visible = FALSE) %>%
  add_lines(data = grid_horas, x = ~Horas_Internet, y = ~Pred_Fair,
            line = list(color = "#e74c3c", width = 3), name = "Tendencia", visible = FALSE) %>%

  # -- GRUPO 3: AYUDA --
  add_markers(data = sd_scatter, x = ~Horas_Jitter, y = ~Help_Jitter,
              marker = list(size = 5, color = "#27ae60", opacity = 0.3),
              name = "Encuestados", visible = FALSE) %>%
  add_lines(data = grid_horas, x = ~Horas_Internet, y = ~Pred_Help,
            line = list(color = "#e74c3c", width = 3), name = "Tendencia", visible = FALSE) %>%

  layout(
    title = "Relación Internet vs Variables Sociales",
    xaxis = list(title = "Horas Internet", range = c(0, 24)),
    ...
    updatemenus = list(
      list(
        y = 1.15, x = 0.1,
        buttons = list(
          list(method = "update", args = list(list(visible = c(T,T,F,F,F,F)), list(title = "Internet vs. Confianza")), label = "Confianza"),
          list(method = "update", args = list(list(visible = c(F,F,T,T,F,F)), list(title = "Internet vs. Justicia")), label = "Justicia"),
          list(method = "update", args = list(list(visible = c(F,F,F,F,T,T)), list(title = "Internet vs. Ayuda")), label = "Ayuda")
        )
      )
    )
  )
