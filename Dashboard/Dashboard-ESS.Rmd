...
# 1. Diccionario de Países (Tu código)
nombres_paises <- c(
...
# 2. Calcular las 3 medias a la vez
...
# Asegúrate de que 'mapa_mundo' ya tiene el fix de Noruega que hicimos antes
...
datos_scatter_full <- ess_data %>%
  filter(essround == 11) %>%
  select(netustm, ppltrst, pplfair, pplhlp) %>%
  # Convertir a numérico
  mutate(across(c(netustm, ppltrst, pplfair, pplhlp), as.numeric)) %>%
  # Filtros de calidad (0-10 y max 24h internet)
  filter(
    netustm <= 1440,
...
# 2. Calcular líneas de tendencia (Regresión Lineal)
# Calculamos los modelos para dibujar las líneas rojas manualmente
...
leaflet(data = mapa_completo) %>%
  ...
  addPolygons(
    ...
    group = "Confianza Interpersonal", # Nombre para el botón
  ...
  addPolygons(
    ...
    weight = 1, color = "white", fillOpacity = 0.9,
    group = "Justicia Social", # Nombre para el botón
  ...
  addPolygons(
    ...
    weight = 1, color = "white", fillOpacity = 0.9,
    group = "Ayuda Social", # Nombre para el botón
...
library(tidyverse)
library(plotly)
library(crosstalk) # <--- Esta es la que faltaba para que funcione SharedData
...
shared_data <- SharedData$new(datos_mariposa)

# Creamos el objeto gráfico 'p'
p <- plot_ly(shared_data, y = ~Nombre_Pais) %>%
  add_trace(
...
bscols(
  ...
  list(
    div(style = "margin-bottom: 20px; margin-left: 10px;", 
       filter_select("region", "Filtrar por Región:", shared_data, ~Region)
      )
  ),
  p
)
...
plot_ly() %>%
  ...
  # ------------------------------------------------------------
  layout(
    title = "Uso de Internet vs. Capital Social",
...
plot_ly() %>%
  ...
  # ------------------------------------------------------------
  layout(
    title = "Consumo de Noticias vs. Capital Social",
